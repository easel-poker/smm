...
def send_tg_file(accounts_list, caption):
    """ارسال فایل متنی پکیج اکانت‌ها به تلگرام"""
    try:
        filename = f"onesmm_accounts_{len(accounts_list)}_{int(time.time())}.txt"
        file_text = f"=== لیست {len(accounts_list)} اکانت ثبت‌شده OneSMM (سرور Railway) ===\n"
        ...
        print(f"[!] خطای ارسال فایل: {e}")


def telegram_listener():
    """پایش و پاسخ‌دهی فوری به پیام‌های تلگرام در پس‌زمینه"""
    global is_running, last_update_id, current_accounts
    print("🤖 سیستم گوش دادن به دستورات تلگرام فعال شد...")

    # پاک‌سازی وبهوک قبلی برای دریافت پیام‌ها
    try:
        requests.get(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/deleteWebhook", timeout=5)
    except Exception:
        pass

    send_tg_msg("🟢 *ربات OneSMM با موفقیت روی سرور Railway روشن شد!*\nآماده دریافت دستورات است.")
        ...
        try:
            ...
            if res.get("ok") and res.get("result"):
                for update in res["result"]:
                    ...
                    print(f"📩 دستور دریافت شد: {text}")

                    if text in ["/start", "▶️ شروع چرخه"]:
                        with lock:
                            is_running = True
                        send_tg_msg(f"🚀 *چرخه خودکار فعال شد!*\nبه ازای هر {BATCH_SIZE} اکانت، یک فایل متنی ارسال خواهد شد.")

                    elif text in ["/stop", "⏹️ توقف چرخه"]:
                        with lock:
                            is_running = False
                        send_tg_msg(f"⏹️ *چرخه خودکار متوقف شد.*\nتعداد فعلی در صف: *{len(current_accounts)} / {BATCH_SIZE}*")
...
def browser_worker():
    """هسته ثبت‌نام در مرورگر"""
    global is_running, current_accounts, total_created_count

    print("🌐 در حال راه‌اندازی مرورگر Playwright روی سرور...")
    ...
    with sync_playwright() as p:
        ...
        while True:
            ...
            print(f"\n[+] در حال ثبت‌نام اکانت جدید: {username}")
            ...
            try:
                ...
                # بررسی رسیدن به ۲۵ اکانت
                if len(current_accounts) >= BATCH_SIZE:
                    print(f"🎉 پکیج {BATCH_SIZE} تایی تکمیل شد! ارسال به تلگرام...")
                    send_tg_file(
                        current_accounts,
                        f"🎁 *پکیج {BATCH_SIZE} تایی اکانت‌ها با موفقیت تکمیل و ارسال شد!*\n🔄 حافظه ریست شد و چرخه ادامه دارد..."
                    )
                    with lock:
                        current_accounts.clear()

            except Exception as e:
                print(f"[!] خطا در پروسه: {e}")
...
if __name__ == "__main__":
    # ۱. اجرای پایش تلگرام در Thread موازی (پاسخ‌دهی زیر ۱ ثانیه)
